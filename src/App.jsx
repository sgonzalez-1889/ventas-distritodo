...
// ---- Resumen consolidado por categoría (total de la empresa) ----
const CAT_COLORES = {
  "MATERIALES": "#ff8a3d",
  "HOGAR": "#00f5a0",
  "MOVILIDAD ELECTRICA": "#00e5ff",
  "MOVILIDAD COMBUSTION": "#b07cff",
};
function ResumenCategorias({ puntos }) {
  const ORDEN = ["MATERIALES", "HOGAR", "MOVILIDAD ELECTRICA", "MOVILIDAD COMBUSTION"];
  // Sumar ventas y meta de cada categoría a través de todos los puntos de venta
  const acc = {};
  (puntos || []).forEach((p) => {
    (p.categorias || []).forEach((c) => {
      const k = String(c.categoria).toUpperCase().trim();
      if (!acc[k]) acc[k] = { ventas: 0, meta: 0 };
      acc[k].ventas += c.ventas || 0;
      acc[k].meta += c.meta || 0;
    });
  });
  const cats = ORDEN.filter((k) => acc[k]).map((k) => ({ categoria: k, ...acc[k] }));
  if (!cats.length) return null;

  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10, width: "100%" }}>
      {cats.map((c) => {
        const col = CAT_COLORES[c.categoria] || "#7d93b8";
        const cumpl = c.meta ? (c.ventas / c.meta) * 100 : null;
        const proy = c.meta && fraccionMes ? ((c.ventas / fraccionMes) / c.meta) * 100 : null;
        return (
          <div key={c.categoria} className="glass" style={{ padding: "12px 14px", borderLeft: `3px solid ${col}` }}>
            <div className="mono" style={{ fontSize: 10, color: col, letterSpacing: .5, textTransform: "uppercase", fontWeight: 600, marginBottom: 6, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.categoria}</div>
            <div style={{ fontSize: 17, fontWeight: 700, color: "#eaf2ff" }}>{fmt(c.ventas)}</div>
            <div className="mono" style={{ fontSize: 10, color: "#7d93b8", marginTop: 2 }}>Ppto: {fmt(c.meta)}</div>
            <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
              <div>
                <div className="mono" style={{ fontSize: 8, color: "#5f7399", textTransform: "uppercase" }}>Cumpl.</div>
                <div className="mono" style={{ fontSize: 13, fontWeight: 700, color: cumpl == null ? "#4a5b80" : colorCierre(proy ?? cumpl) }}>{cumpl == null ? "—" : cumpl.toFixed(0) + "%"}</div>
              </div>
              <div>
                <div className="mono" style={{ fontSize: 8, color: "#5f7399", textTransform: "uppercase" }}>Cierre</div>
                <div className="mono" style={{ fontSize: 13, fontWeight: 700, color: proy == null ? "#4a5b80" : colorCierre(proy) }}>{proy == null ? "—" : proy.toFixed(0) + "%"}</div>
              </div>
            </div>
          </div>
        );
...
export default function App() {
  ...
  const unirCategorias = (registros, mapaCategorias) => {
    ...
    return registros.map((r) => {
      ...
      if (!cats || !cats.length) return r;
      // La venta y meta totales se calculan sumando las categorías (una sola fuente de verdad)
      const ventasCat = cats.reduce((s, c) => s + (c.ventas || 0), 0);
      const metaCat = cats.reduce((s, c) => s + (c.meta || 0), 0);
      return {
        ...r,
        categorias: cats,
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#05080f", color: "#dfe7f5", position: "relative",
      ...
      <div style={{ maxWidth: 1140, margin: "0 auto", position: "relative", opacity: mounted ? 1 : 0, transition: "opacity .6s" }}>
        <header style={{ marginBottom: 30, animation: "rise .6s both", display: "flex", justifyContent: "space-between", gap: 24, flexWrap: "wrap", alignItems: "flex-start" }}>
          ...
          {/* Resumen por categoría — total de la empresa (puntos de venta) */}
          <div style={{ flex: "1 1 360px", minWidth: 300, maxWidth: 520 }}>
            <div className="mono" style={{ fontSize: 11, color: "#7d93b8", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10, display: "flex", alignItems: "center", gap: 7 }}>
              <Activity size={13} color="#00e5ff" /> Resumen por categoría · empresa
            </div>
            <ResumenCategorias puntos={puntos} />
          </div>
        </header>
