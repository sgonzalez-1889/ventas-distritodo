...
function Seccion({ titulo, icono, acentoColor, items, setItems, editKey, edit, setEdit, draft, setDraft, mostrarPV, admin, persistir }) {
...
export default function App() {
  ...
  const cargarExcel = async (e) => {
    ...
    try {
      ...
      if (ase && ase.length) { nuevosAse = ase; setAsesores(ase); msg.push(`${ase.length} asesores`); }
      if (pv && pv.length) { nuevosPv = pv; setPuntos(pv); msg.push(`${pv.length} puntos de venta`); }
