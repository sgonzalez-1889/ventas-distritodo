...
import {
  Plus, Trash2,
  Edit3, X, Check, Upload, AlertCircle, Activity, Zap, Store, Users, Lock, LogOut
} from "lucide-react";
...
function Seccion({ titulo, icono, acentoColor, items, setItems, editKey, edit, setEdit, draft, setDraft, mostrarPV, admin }) {
  ...
  const guardar = () => {
    setItems((prev) => prev.map((a) => a.id === edit.id ? { ...draft, meta:+draft.meta, ventas:+draft.ventas, cotPendientes:+draft.cotPendientes, cotMonto:+draft.cotMonto } : a));
    setEdit(null);
  };
  const agregar = () => {
    ...
    setItems((p) => [...p, nuevo]); abrir(nuevo);
  };
...
export default function App() {
  ...
  const cargarExcel = async (e) => {
    ...
    try {
      ...
      let msg = [];
      if (ase && ase.length) { setAsesores(ase); msg.push(`${ase.length} asesores`); }
      if (pv && pv.length) { setPuntos(pv); msg.push(`${pv.length} puntos de venta`); }
      if (!msg.length) throw new Error("No se encontraron datos válidos.");
      setAviso({ tipo: "ok", txt: `Cargado: ${msg.join(" y ")}.` });
    } catch (err) {
    ...
    <div style={{ minHeight: "100vh", background: "#05080f", color: "#dfe7f5", position: "relative",
      ...
      <div style={{ maxWidth: 1140, margin: "0 auto", position: "relative", opacity: mounted ? 1 : 0, transition: "opacity .6s" }}>
        <header style={{ marginBottom: 30, animation: "rise .6s both" }}>
          <div className="mono" style={{ display: "flex", alignItems: "center", gap: 8, color: "#00e5ff", fontSize: 12, letterSpacing: 2, textTransform: "uppercase" }}>
        ...
        <Seccion titulo="Asesores de ventas" icono={<Users />} acentoColor="#00e5ff"
          ...
          edit={edit} setEdit={setEdit} draft={draft} setDraft={setDraft} />
